		...
		if (data.length) {
			...
			database.query(check_author_query, (error, data) => {
				...
				if (!data.length) {
					...
					const get_post_by_id_query = "SELECT `users_info`.`name` AS `post_author_name`, `users_info`.`profile_picture` AS `post_author_avatar`, `user_posts`.`post_id`, `user_posts`.`post_title`, `user_posts`.`post_content`, `user_posts`.`post_date`, `user_posts`.`post_author`, `user_posts`.`post_type` " +
